...
import { Lock, Mail, Cpu, RefreshCw, AlertCircle, Eye, EyeOff } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response?.error) {
        setError(response.error.message || 'Failed to sign in. Please verify credentials.');
      } else {
        triggerToast('Login successful! Redirecting...');
        setTimeout(() => {
          router.push('/');
          router.refresh();
        }, 800);
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center items-center px-4 py-12">
      ...
      <div className="glass-panel w-full max-w-md p-8 rounded-2xl border border-slate-800 shadow-2xl">
        ...
        <p className="text-xs text-slate-400 mb-6">Enter your credentials to access the agent workspace.</p>
